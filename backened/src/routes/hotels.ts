...
router.get("/search", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // page number = 3 then 2*5 = 10 hotels to be skipped 
...
router.post("/:hotelId/bookings/payment-intent", verifyToken, async (req: Request, res: Response) => {
  ...
  const paymentIntent = await stripe.paymentIntents.create({ amount: totalCost * 100, currency: "inr", metadata: { hotelId, userId: req.userId } });
